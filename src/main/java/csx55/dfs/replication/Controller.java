...
public class Controller implements Node {
    ...
    private synchronized void handleRegistrationEvent(Register registerEvent, TCPConnection connection) {
        ...
        if (!hasMismatch) {
            ...
            message = "Unable to process request. Responding with a failure while ping mismatch is" + hasMismatch;
    ...
    private synchronized void handleDeregistrationEvent(Register registerEvent, TCPConnection connection) {
        ...
        if (!hasMismatch && connections.containsKey(nodes)) {
            ...
            message = "De-registration request successful.  The number of messaging nodes currently "
                    + "constituting the overlay is (" + connections.size() + ").\n";
            status = Protocol.SUCCESS;
        } else {
            message = "Unable to process de-registration request. Responding with a failure. Mismatch and connection exists? "
